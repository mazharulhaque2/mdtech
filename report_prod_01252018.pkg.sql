...
create or replace PACKAGE report_pkg IS
  ...
  PROCEDURE report_patient_med (p_locale in varchar2,p_patient_sn in number,p_out OUT clob);
  --
...
create or replace PACKAGE BODY report_pkg IS
  ...
  begin
    for i in (select case 
              ...
              where srv.patient_prev_svc_sn = prv.patient_prev_svc_sn(+)
  ...
  --
  function strk_rf(p_patient_prev_svc_sn in number,p_age in number,p_primary_rf_flag in varchar2) return json_list
  is
    l_obj json_list := json_list();
    l_obj2 json_list := json_list();
  begin
  ...
  begin
    ...
    if common_inq_pkg.patient_question_response_flag (p_patient_prev_svc_sn,'FALLR','1002','NOO') = 'Y' then 
      if common_inq_pkg.patient_question_response_flag (p_patient_prev_svc_sn,'HENMT','1002','CBX') = 'Y' then
  ...
  begin
    ...
    if p_disease_code = 'STRK' then
      l_obj := strk_rf(p_patient_prev_svc_sn,p_age,p_primary_rf_flag);
    else
      l_obj := json_list();
  ...
  begin
    ...
    if p_bmi_result = 'Underweight' then
      v_outcome_text := 'Body mass index BMI is less than 18.5. Your BMI indicated you are **Underweight**.';
      v_other_outcome_text := 'In view your BMI today is '||p_bmi||' your Healthcare Professional want you become aware what your Risks of a low body weight without a known cause, pose your Health in an out numbers of serious Medical conditions as  Anemia & Nutrient Deficiencies, Osteoporosis, Heart irregularities, Blood Vessel Disease, Prone to Infections & disease, delayed wound healing among others serious conditions.';
    elsif p_bmi_result = 'Overweight' then
      v_outcome_text := 'Body mass index BMI is between 25 and 29.9. Your BMI indicated you are **Overweight**.';
      if p_qualify_for_em_followup_flag = 'N' then
        v_other_outcome_text := 'Your BMI '||p_bmi||' indicated you are Overweight. Your PMHX and HRA outcome does not show any additional disease related Risk Factors that may influence a major Health issue. Regardless your Healthcare Professional recommend reviewing your eating habits and Life style to improve a healthier weight to prevent a harmful obesity.';
      else --Y
        v_other_outcome_text := 'Your BMI '||p_bmi||' indicated you are Overweight. PMHX and HRA outcome does show additional serious Disease Related Risk Factors as You are at Higher Risk to get exacerbated a Major Disease';
      ...
      v_outcome_text := 'Body mass index BMI is >= 30. Your BMI indicated you are **Obese**.';
      if p_qualify_for_em_followup_flag = 'N' then
        v_other_outcome_text := 'Your BMI '||p_bmi||' indicated you are Obese. Your PMHX & HRA Outcome did not found other important disease related Risk Factors with your Obesity. Your Healthcare Professional is referring you to an Intensive Behavioral Therapy (IBT) Weight Loss Counseling Program aimed to help you to lose weight free of charge as recommended by Medicare.';
      else --Y
        v_other_outcome_text := 'Your BMI '||p_bmi||' indicated you are Obese. Your PMHX & HRA Outcome found other important disease related Risk Factors with your Obesity as stated in your HRA outcome Report of finding.';
        v_neg_health_finding_text := 'Your PMHX & HRA Outcome found other important disease related Risk Factors with your Obesity as stated in your HRA outcome Report of finding.';
        v_ppp_em_text := 'In view of the afore mentioned clinical reasons a Face to Face Physician Assessment is medically needed to 1-  establish a Preventive Plan of Care aimed to control your weight 2-  establish an appropriate Prevention measurement to avoid or retard all severe Health issues at Risk of exacerbation. 3- Help preventing gaining more weight your Physician may recommend some lab and other necessary analysis & Test to maintain your ideal weight levels for Better Health 4- perform closely review all Disease at Risk causes for an adequate Preventive Plan and Plan of Care.';
        v_ppp_ccm_text := 'At the Physician''s clinical judgement, you may qualify with your consent to a CCM (Chronic Care Management) monthly monitoring program for a better control of your Health.';
      end if;
    end if;
...
--    end if;
  ...
  begin
    ...
    obj2.put('SUB_TITLE7','THE FOLLOWS RF MAY HAVE INFLUENCED MR./MRS. PERSONALIZED HRA EVIDENCED CLINICAL DISEASE SCORED OUTCOME');
    obj.put('hdr',obj2);
    --=======================pmhx_label
    obj2 := json(); --an empty structure
...
--    obj2.put('para5','has been prepared by the physician to be follow at your discretion.');
  ...
  begin
    ...
    for i in (select patient_sn
                    ...
                    ,age||'y/o' pat_age
--                    ,bmi||'('||bmi_result||')' pat_bmi
--                    ,height_in_ft pat_height_in_ft
--                    ,weight_in_lb||' LB' pat_weight_in_lb
--                    ,hdl_cholesterol_in_mg pat_hdl_cholesterol_in_mg
--                    ,ldl_cholesterol_in_mg pat_ldl_cholesterol_in_mg
--                    ,systolic_bp_in_mm||'/'||diastolic_bp_in_mm pat_bp
              from patient_vw
              where patient_sn = p_patient_sn
              )
    loop
...
--      obj.put('sub_hdr', obj2);
      ...
      --
      report_pkg.report_patient_med(p_locale,p_patient_sn,v_rpt_out);
      v_rpt_obj := json();
